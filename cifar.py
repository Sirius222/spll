...
class MY_CIFAR10_one_hot(VisionDataset):
    """`CIFAR10 <https://www.cs.toronto.edu/~kriz/cifar.html>`_ Dataset.
    ...
    def __init__(self, root, train=True, transform=None, target_transform=None,
                 download=False, rate_partial=0.3, label_dir=None, index=None, pll_label=None):

        super(MY_CIFAR10_one_hot, self).__init__(root, transform=transform,
                                         target_transform=target_transform)
        ...
        self.rate_partial = rate_partial
        # self.partial_labels = self.generate_partial_labels()
        # pll_path = os.path.join(self.root, 'PLL_label.pt')
        # self.partial_labels = torch.load(pll_path).numpy()
        # self.partial_labels = self.partial_labels[index]
        self.partial_labels = pll_label[index]
