...
class ConfigParser:
    ...
    def __init__(self, args, options='', timestamp=True):
        ...
        # fixed some bugs on jupyter notebook
        args = args.parse_args()
        ...
        elif self.config['trainer']['sys']:
            exper_name = self.config['name'] + '_sym_' + str(int(self.config['trainer']['percent']*100))
        else: # defalut real noise
            exper_name = self.config['name'] + '_real_' + str(self.config['trainer']['real'])
        # else:
        #     exper_name = self.config['name'] + '_sym_' + str(int(self.config['trainer']['percent']*100))
        self._save_dir = save_dir / 'models' / exper_name / timestamp
        self._log_dir = save_dir / 'log' / exper_name / timestamp
        self._tb_logger = save_dir / 'tensorboard' / exper_name / timestamp
        self._pll_label = save_dir / 'pll_label' / exper_name / timestamp

        self.save_dir.mkdir(parents=True, exist_ok=True)
        self.log_dir.mkdir(parents=True, exist_ok=True)
        self.tb_logger.mkdir(parents=True, exist_ok=True)
        self.pll_label.mkdir(parents=True, exist_ok=True)
