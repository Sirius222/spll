...
from base import PLL_BaseTrainer
from utils import inf_loop
# import sys
import torch.nn.functional as F
# import tensorboard_logger as tb_logger

class  PLL_Trainer(PLL_BaseTrainer):
    """
    Trainer class
    ...
    def __init__(self, model, reparametrization_net, train_criterion, metrics, optimizer, optimizer_loss, config, data_loader,
                 valid_data_loader=None, test_data_loader=None, lr_scheduler=None, lr_scheduler_overparametrization=None, len_epoch=None, val_criterion=None,tb_logger=None):
        ...
        self.ens_example_loss = np.zeros((self.epochs,self.samples), dtype=float) # (300, 50000)
        self.ens_prediction = np.zeros((self.epochs,self.samples,self.classes), dtype=float) # (300, 50000, 10)
    ...
    def _train_epoch(self, epoch):
        ...
        noise_level = 0
        example_loss = np.zeros((self.samples,), dtype=float)
        example_prediction = np.zeros((self.samples,self.classes), dtype=float)
        with tqdm(self.data_loader) as progress:
            # for batch_idx, (data, data2, label, indexs, _) in enumerate(progress):
            for batch_idx, (data, data2, label, indexs, gt) in enumerate(progress):
                progress.set_description_str(f'Train epoch {epoch}')


                data, label = data.to(self.device), label.long().to(self.device)
                # data, label, gt = data.to(self.device), label.long().to(self.device), gt.to(self.device)
                ...
                loss, loss_ce, prediction_ce = self.train_criterion(indexs, output, target)

                for idx, l, p in zip(indexs, loss_ce, prediction_ce):
                    example_loss[idx] = l.detach().cpu().numpy()
                    example_prediction[idx] = p.detach().cpu().numpy()
        ...
        self.ens_example_loss[epoch-1] = example_loss
        self.ens_prediction[epoch-1] = example_prediction
        ...
        self.tb_logger.log_value("train loss", log['loss'], epoch)
        self.tb_logger.log_value("train Top1", log['metrics'][0], epoch)
        ...
        self.tb_logger.log_value("test loss", test_log['test_loss'], epoch)
        self.tb_logger.log_value("test Top1", test_log['test_metrics'][0], epoch)
